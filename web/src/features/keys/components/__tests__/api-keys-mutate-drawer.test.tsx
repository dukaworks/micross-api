...
describe('API keys mutate drawer group routing', () => {
  // Group selection is not offered to the client, so the routing payload must
  // come from the server-side default alone: every key of a batch inherits the
  // global Auto order and Auto cross-group retry, with no per-key override.
  test('creates every key of a batch with the inherited Auto routing payload', async () => {
    ...
    for (const payload of createdPayloads) {
      ...
      // An empty override is the wire signal for "inherit the global order".
